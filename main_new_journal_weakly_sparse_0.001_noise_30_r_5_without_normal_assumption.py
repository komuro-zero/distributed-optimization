...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.04,0.01,1.1,self.iteration)
        self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.28/self.m,0.05,0.09,self.iteration,graph,w_all)
        self.pg_extra_mc_consensus_violation(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.4/self.m,1,9.7/self.m,self.iteration,graph,w_all,0,0.15)
        # # self.pg_extra_mc_consensus_violation(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.04/self.m,1,1.1/self.m,self.iteration,graph,w_all,10,0.08)
        self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.34/self.m,0.05,0.88/self.m,self.iteration,graph,w_all)
        ...
        plt.ylabel("Average distance from the opti (dB)",fontsize=12)
        plt.grid(which = "major")
        # plt.axhline(y = error[-1],linestyle="dashed",label = "Centralized L1 penalty",color = "green")
        # plt.axhline(y = error_centralized_mc[-1],linestyle="dashed",label = "Centralized MC penalty",color = "green")

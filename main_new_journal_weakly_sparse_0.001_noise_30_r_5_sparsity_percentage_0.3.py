...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.4,0.01,9.7,self.iteration)
        self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.21/self.m,0.016,0.09,self.iteration,graph,w_all)
        self.pg_extra_mc_consensus_violation(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.4/self.m,1,9.7/self.m,self.iteration,graph,w_all,0,0.07)
        self.pg_extra_mc_consensus_violation(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.4/self.m,1,9.7/self.m,self.iteration,graph,w_all,10,0.04)
        self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1/self.m,0.016,4.6/self.m,self.iteration,graph,w_all)
        self.prox_dgd(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.4/self.m,0.01,9.7/self.m,self.iteration,graph,w_all)
        ...
        plt.ylabel("Average distance from the optimal vector (dB)",fontsize=12)
        plt.grid(which = "major")
        # plt.axhline(y = error[-1],linestyle="dashed",label = "Centralized L1 penalty",color = "green")
        plt.axhline(y = error_centralized_mc[-1],linestyle="dashed",label = "Centralized MC penalty",color = "green")

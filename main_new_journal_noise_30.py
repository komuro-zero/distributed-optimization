...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,1.4,0.01,3.3,self.iteration)
        error_centralized_scad,wcmc = self.centralized_scad(U_all,d_all,w,w_star,L2,0.05,0.01,7.9,self.iteration)
        self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.19/self.m,0.02,0.09,self.iteration,graph,w_all)
        self.pg_extra_mc_consensus_violation(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.4/self.m,1,3.3/self.m,self.iteration,graph,w_all,0,1)

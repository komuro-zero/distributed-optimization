...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        # plt.ylabel("2nd smallest eigenvalue of VVT",fontsize=12)
        ...
        # plt.show()
        # error_dgd,wdgd = self.centralized_gradient_descent(U_all,d_all,w,w_star,L2,0.07,self.iteration)
        # error_l1,wl1 = self.centralized_L1(U_all,d_all,w,w_star,L2,0.0,0.07,self.iteration)
        # error_mc,wcmc = self.centralized_partial_mc(U_all,d_all,w,w_star,L2,0.63,0.01,0.9,self.iteration,self.m)
        # error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.9,0.01,2,self.iteration)
        error_l1,wl1 = self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.005/self.m,0.02,0.09,self.iteration,graph,w_all)


        # self.centralized_partial_mc(U_all,d_all,w,w_star,L2,0.9,0.01,2,self.iteration,self.m)
        ...
        #     # error_l1,wl1 = self.centralized_L1(U_all,d_all,w,w_star,L2,lamb,0.07,self.iteration)
        ...
        for i in range(10):
            lamb =0.01+0.01*i
            # error_centralized_mc,wcmc = self.centralized_partial_mc(U_all,d_all,w,w_star,L2,lamb,0.01,1,self.iteration,self.m)
            error_centralized_mc,w = self.pg_extra_partial_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,lamb/self.m,1,1/self.m,self.iteration,graph,w_all)
            if error_centralized_mc != None:
                if thresh > error_centralized_mc[-1]:
        ...
        # self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.0003/self.m,0.02,0.09,self.iteration,graph,w_all)
        # self.pg_extra_partial_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.63/self.m,1,0.9/self.m,self.iteration,graph,w_all)
        ...
        #         lamb = 0.05+0.05*i
        #         rho = 0.1+0.1*j
        #         error,w = self.centralized_partial_mc(U_all,d_all,w,w_star,L2,lamb,0.01,rho,self.iteration,self.m)
        #         # error,w = self.pg_extra_partial_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,lamb/self.m,1,rho/self.m,self.iteration,graph,w_all)
        #         if error[-1] <  thresh:
        #             thresh = error[-1]
        #             optimal_lamb = lamb
        #             optimal_rho = rho
        ...
        plt.ylabel("Average Disagreement (dB)",fontsize=12)
        plt.grid(which = "major")
        # plt.axhline(y = error[-1],linestyle="dashed",label = "Centralized L1 penalty",color = "green")
        # plt.axhline(y = error_l1[-1],linestyle="dashed",label = "Centralized l1 penalty",color = "green")
        # plt.axhline(y = error_centralized_mc[-1],linestyle="dashed",label = "Centralized MC penalty",color = "green")
        # plt.axhline(y = error_centralized_dual_soft[-1],linestyle = "dashed",label = "Centralized Approximate MC penalty",color = "purple")
        # pdf.savefig()
        plt.legend(fontsize=12)
        plt.savefig('journal_global_convergence_2.pdf')
        plt.show()
        x  = range(len(w_star))
        plt.plot(x,wdgd,label = "dgd")
        plt.plot(x,wl1,label = "L1")

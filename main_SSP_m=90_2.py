    ...
    def run(self):
        ...
        for i in range(100):
            for j in range(100):
                lamb = 0.01 + 0.01*i
                rho = 0.1 + 0.1*j
                error_centralized_smc,wcsmc = self.centralized_mc_twin_with_amc(U_all,d_all,w,w_star,L2,lamb,0.001,rho,self.iteration,self.m)
        ...
        #         lamb = 0.1 + 0.1*i
        #         rho = 0.1 + 0.1*j
        #         error,w_approximate = self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,lamb/self.m,0.04,rho/self.m,self.iteration,graph,w_all)
        ...
        # error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.6,0.001,1.4,self.iteration)
        ...
        # self.extra(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,self.lamb,0.02,self.rho,self.iteration,graph,w_all)
        ...
        # error_amc,w_amc  = self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.44/self.m,0.04,0.52/self.m,self.iteration,graph,w_all)
        # print("w difference", np.linalg.norm(wcmc-w_amc))
        # print("objective difference", np.linalg.norm(error_amc-error_centralized_mc))

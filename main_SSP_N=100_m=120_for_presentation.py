...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.29,0.001,1.3,self.iteration)
        # # error_l1,wl1 = self.centralized_L1(U_all,d_all,w,w_star,L2,0.1,10,self.iteration)

        # error_centralized_mc_twin,wcmc_twin = self.centralized_mc_twin(U_all,d_all,w,w_star,L2,0.25,0.001,0.09,10000,self.m)
        self.extra(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,self.lamb,0.001,self.rho,self.iteration,graph,w_all)
        self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.23/self.m,0.001,0.09,self.iteration,graph,w_all)
        self.pg_extra_mc_nonconvex_twin(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.29/self.m,0.001,1.3/self.m,self.iteration,graph,w_all)
        self.prox_dgd(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.29/self.m,0.001,1.3/self.m,self.iteration,graph,w_all)
        error_amc,wamc=self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.27/self.m,0.001,0.5/self.m,self.iteration,graph,w_all)
        # print("w difference", np.linalg.norm(wcmc-wamc))
        # print("objective difference", np.linalg.norm(error_amc[-1]-error_centralized_mc[-1]))

...
class distributed_updates(functions):
    ...
    def run(self):
        ...
        # self.distributed_convexity_checker(10,self.lamb/self.m,U_all,self.N)
        wdmc = self.distributed_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,self.lamb/self.m,0.0008,(self.lamb/self.m)*((0.1)**2),self.iteration,graph,w_all)
        self.distributed_mc_compare(U_all,d_all,wcmc,L2,self.N,self.m,self.r_i,self.lamb/self.m,0.0008,(self.lamb/self.m)*((1.5)**2),self.iteration,graph,w_all)
        ...
        plt.plot(x,wcmc,label = "cmc")
        plt.plot(x,wdmc,label = "dmc")

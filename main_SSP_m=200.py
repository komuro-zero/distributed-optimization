...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.4,0.001,17,10000)
        error_l1,wl1 = self.centralized_L1(U_all,d_all,w,w_star,L2,0.2,10,self.iteration)

        # error_centralized_mc_twin,wcmc_twin = self.centralized_mc_twin(U_all,d_all,w,w_star,L2,0.98,0.001,4.5,10000,self.m)
        ...
        error_amc, w_amc = self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.98/self.m,0.02,4.5/self.m,self.iteration,graph,w_all)
        print("w difference", np.linalg.norm(wcmc-w_amc))
        print("amc difference from wstar", np.linalg.norm(error_amc[-1]))
        print("mc difference from wstar", np.linalg.norm(error_centralized_mc[-1]))

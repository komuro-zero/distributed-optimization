...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        error_m = []
        for i in range(6):
            plt_time.append(10*(i+1))
            w,w_star,w_all,U_all,d_all,L2,graph = self.make_variables_noise_after_2(self.N,(self.m)*(i+1),self.r_i,self.sparsity_percentage,self.how_weakly_sparse,self.w_noise,self.normal_distribution)
            # error,w_non_mc = self.centralized_gradient_descent(U_all,d_all,w,w_star,L2,0.03,self.iteration)
            # error,w_non_mc = self.centralized_L1(U_all,d_all,w,w_star,L2,1.8,0.03,self.iteration)
            if i == 0:
                w_non_mc = self.distributed_proximal_gradient_algorithm_firm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.8/self.m,0.01,3.2/self.m,self.iteration,graph,w_all)
                w_mc = self.distributed_proximal_gradient_algorithm_approximate_MC(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.8/self.m,0.01,3.2/self.m,self.iteration,graph,w_all)

                # error,w_non_mc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,1.8,0.01,3.2,self.iteration,self.m)
                # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,1.8,0.01,3.2,self.iteration)
            if i == 1:
                w_non_mc = self.distributed_proximal_gradient_algorithm_firm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.4/self.m,0.01,3.2/self.m,self.iteration,graph,w_all)
                w_mc = self.distributed_proximal_gradient_algorithm_approximate_MC(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.4/self.m,0.01,3.2/self.m,self.iteration,graph,w_all)

                # error,w_non_mc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,1.4,0.01,3.2,self.iteration,self.m)
                # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,1.4,0.01,3.2,self.iteration)  
            if i == 2:
                w_non_mc = self.distributed_proximal_gradient_algorithm_firm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.35/self.m,0.01,7/self.m,self.iteration,graph,w_all)
                w_mc = self.distributed_proximal_gradient_algorithm_approximate_MC(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.35/self.m,0.01,7/self.m,self.iteration,graph,w_all)

                # error,w_non_mc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,1.35,0.01,7,self.iteration,self.m)
                # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,1.35,0.01,7,self.iteration)
            if i == 3:
                w_non_mc = self.distributed_proximal_gradient_algorithm_firm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,4.8/self.m,0.01,6.4/self.m,self.iteration,graph,w_all)
                w_mc = self.distributed_proximal_gradient_algorithm_approximate_MC(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,4.8/self.m,0.01,6.4/self.m,self.iteration,graph,w_all)
                # error,w_non_mc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,4.8,0.001,6.4,self.iteration,self.m)
                # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,4.8,0.001,6.4,self.iteration)
            if i == 4:
                w_non_mc = self.distributed_proximal_gradient_algorithm_firm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,4.4/self.m,0.01,7.9/self.m,self.iteration,graph,w_all)
                w_mc = self.distributed_proximal_gradient_algorithm_approximate_MC(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,4.4/self.m,0.01,7.9/self.m,self.iteration,graph,w_all)
                # error,w_non_mc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,4.4,0.001,7.9,self.iteration,self.m)
                # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,4.4,0.001,7.9,self.iteration)
            if i == 5:
                w_non_mc = self.distributed_proximal_gradient_algorithm_firm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2.3/self.m,0.01,9.3/self.m,self.iteration,graph,w_all)
                w_mc = self.distributed_proximal_gradient_algorithm_approximate_MC(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2.3/self.m,0.01,9.3/self.m,self.iteration,graph,w_all)

        #         error,w_non_mc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,2.3,0.001,9.3,self.iteration,self.m)
        #         error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,2.3,0.001,9.3,self.iteration)
            error_m.append(np.linalg.norm(w_non_mc-w_mc,ord = 2)/np.linalg.norm(w_non_mc,ord=2))
        plt.plot(plt_time,error_m)
        print(len(plt_time),len(error_m))
        plt.legend()
        plt.show()

        # self.distributed_proximal_gradient_algorithm_firm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2.3/self.m,0.035,9.3/self.m,self.iteration,graph,w_all)
        # self.distributed_proximal_gradient_algorithm_approximate_MC(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2.3/self.m,0.035,9.3/self.m,self.iteration,graph,w_all)

        plt.legend()
        plt.show()
        # error,w_non_mc = self.centralized_gradient_descent(U_all,d_all,w,w_star,L2,0.01,self.iteration)
        # error,w_non_mc = self.centralized_L1(U_all,d_all,w,w_star,L2,0.9,0.01,self.iteration)
        # error,w_non_mc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,2.3,0.01,9.3,self.iteration,self.m)
        # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,2.3,0.01,9.3,self.iteration)
        # plt.legend()
        # plt.show()

...
class functions():
	...
	def w_star_weakly_sparse(self,N,sparse_percentage):
		...
		while (after_length / before_length) > sparse_percentage:
			...
			w_star[a] = 0
			after_length = len(np.unique(w_star))-1  #重複要素の削除
		w_star_prox = [0]*len(w_star)
		for i in range(len(w_star)):
			if w_star[i] == 0:
				w_star_prox[i] = randn()
		length = len(w_star_prox)
		w_star_prox = np.array(w_star_prox,(length,1))
		print(w_star_prox)
		exit()
	...
	def w_average(self,w_all,w_star):
		...
		x = range(len(result))
		plt.scatter(x,result,color = "black",label = "result",s =5)
		plt.scatter(x,w_star,color = "red",label = "w_star", s = 5)
		plt.legend()
		plt.show()
		return result

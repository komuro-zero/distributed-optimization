...
class distributed_updates(functions):
    ...
    def make_variables(self):
        ...
        """
        w = randn(self.N,1)
        w_star = self.w_star_weakly_sparse(self.N,self.sparsity_percentage)
        U_all = randn(self.m,self.N)
        w_star_noise = w_star #+randn(self.N,1)/10000
    ...
    def run(self):
        ...
        self.distributed_gradient_descent(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.2405,self.iteration,c,w_all)
        self.distributed_L1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,10**-17,0.224,self.iteration,c,w_all)

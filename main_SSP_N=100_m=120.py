...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.25,0.001,1.2,100)
        error_l1,wl1 = self.centralized_L1(U_all,d_all,w,w_star,L2,0.21,10,100)
        ...
        # error_l1,wl1=self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.1/self.m,0.02,0.09,self.iteration,graph,w_all)
        # self.pg_extra_mc_nonconvex_twin(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.25/self.m,0.02,1.2/self.m,self.iteration,graph,w_all)
        # self.prox_dgd(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.4/self.m,0.02,17/self.m,self.iteration,graph,w_all)
        error_amc,wamc=self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.23/self.m,0.01,0.6/self.m,self.iteration,graph,w_all)
        I = np.eye(self.N)
        I_approximation = U_all.T@U_all/self.m

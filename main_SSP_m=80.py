...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        while i <trials:
            B = (2.7/0.53)**0.5
            w,w_star,w_all,U_all,d_all,L2,graph = self.make_variables_noise_after_2(self.N,self.m,self.r_i,self.sparsity_percentage,self.how_weakly_sparse,self.w_noise,self.normal_distribution,self.w_zero)
            if self.centralized_convexity_checker(B,0.53,U_all,len(w)):
                error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.53,0.001,2.7,self.iteration)
                # error_l1,wl1 = self.centralized_L1(U_all,d_all,w,w_star,L2,0.17,10,self.iteration)
                ...
                error_l1,w_l1=self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.17/self.m,0.02,0.09,self.iteration,graph,w_all,0,1)
                ...
                error_amc,wamc  = self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.29/self.m,0.04,0.34/self.m,self.iteration,graph,w_all,1)

    ...
    def run(self):
        ...
        while i <trials:
            ...
            if self.centralized_convexity_checker(B,0.37,U_all,len(w)):
                ...
                error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.37,0.001,0.42,self.iteration)
                # error_l1,wl1 = self.centralized_L1(U_all,d_all,w,w_star,L2,0.13,10,self.iteration)
                ...
                error_l1,w_lw = self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.13/self.m,0.02,0.09,self.iteration,graph,w_all,0,1)
                ...
                error_amc, wamc = self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.22/self.m,0.04,0.25/self.m,self.iteration,graph,w_all,1)
                amc_list.append(error_amc[-1])
                mc_list.append(error_centralized_mc[-1][0])
                l1_list.append(error_l1[-1])

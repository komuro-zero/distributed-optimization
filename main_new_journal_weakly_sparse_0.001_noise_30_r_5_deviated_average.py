...
class distributed_updates(update_functions):
    ...
    def run(self):
        w,w_star,w_all,U_all,d_all,L2,graph = self.make_variables_noise_after_2_deviated_average(self.N,self.m,self.r_i,self.sparsity_percentage,self.how_weakly_sparse,self.w_noise,self.normal_distribution,self.w_zero,0.1)
        ...
        # plt.xlabel("Neighboring nodes",fontsize=18)
        ...
        # print(U_all.T@U_all)
        # print(np.average(U_all),np.std(U_all))   
        error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.36,1,14,6000)
        self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.34/self.m,1,0.8/self.m,self.iteration,graph,w_all,0.1)
        self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.29/self.m,0.03,0.09,self.iteration,graph,w_all,1,0.6)
        # self.pg_extra_mc_consensus_violation(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.65/self.m,1,7.6/self.m,self.iteration,graph,w_all,0,0.15)
        self.pg_extra_mc_consensus_violation(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.36/self.m,1,14/self.m,self.iteration,graph,w_all,10,0.08)
        # self.prox_dgd(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.9/self.m,0.007,2/self.m,self.iteration,graph,w_all)
        ...
        #         lamb = 0.01 + 0.01*i
        #         rho =  0.01 + 0.01*j
        #         error,w = self.pg_extra_approximate_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,lamb/self.m,0.05,rho/self.m,self.iteration,graph,w_all)
        #         if error != None:
        #             if error[-1] < thresh:
        #                 thresh = error[-1]
        #                 optimal_lamb = lamb
        ...
        #         lamb = 0.01 + 0.01*i
        #         rho =  1 + 1*j
        #         error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,lamb,0.01,rho,6000)
        ...
        #         lamb = 0.01 + 0.01*i
        #         rho = 0.1 + 0.1*j
        #         error_centralized_smc,wcsmc = self.centralized_mc_twin_with_amc(U_all,d_all,w,w_star,L2,lamb,0.001,rho,600,self.m)
        ...
        plt.savefig('journal_system_mismatch_with_l1_and_amc_deviated_average.pdf')
        plt.show()
        # x  = range(len(w_star))

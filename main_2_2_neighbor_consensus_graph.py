...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        # self.distributed_proximal_gradient_algorithm_firm_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2/self.m,0.03,5/self.m,self.iteration,graph,w_all)
        # self.distributed_proximal_gradient_algorithm_consensus_violation_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2/self.m,0.03,5/self.m,self.iteration,graph,w_all)
        
        


        # self.prox_dgd_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.8/self.m,0.018,8.2/self.m,self.iteration,graph,w_all)
        # self.prox_dgd_consensus_violation_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.8/self.m,0.018,8.2/self.m,self.iteration,graph,w_all)

        self.pg_extra_mixing_matrix_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.2/self.m,0.02,0.09,self.iteration,graph,w_all,0,1)
        self.pg_extra_mixing_matrix_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.2/self.m,0.02,0.09,self.iteration,graph,w_all,0.1,1)
        self.pg_extra_mixing_matrix_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.2/self.m,0.02,0.09,self.iteration,graph,w_all,1,1)
        self.pg_extra_mixing_matrix_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.2/self.m,0.02,0.09,self.iteration,graph,w_all,0,0.1)
        self.pg_extra_mixing_matrix_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.2/self.m,0.02,0.09,self.iteration,graph,w_all,0.1,0.1)
        self.pg_extra_mixing_matrix_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.2/self.m,0.02,0.09,self.iteration,graph,w_all,1,0.1)
        # self.pg_extra_mc_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2.2/self.m,0.02,0.09,self.iteration,graph,w_all)
        # self.pg_extra_mc_consensus_violation_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2.2/self.m,0.02,0.09,self.iteration,graph,w_all,5)
        # self.pg_extra_mc_consensus_violation_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2.2/self.m,0.02,0.09,self.iteration,graph,w_all,0)
        ...
        # self.pg_extra_mixing_matrix_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,1.2/self.m,0.035,0.09,self.iteration,graph,w_all,0.5,0.1)
        # self.pg_extra_mc_consensus_violation_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2.2/self.m,0.02,0.09,self.iteration,graph,w_all,1)
        ...
        # error,w_non_mc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,2,0.001,5.05,self.iteration,self.m)
        # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,2,0.001,5.05,self.iteration)
        
        # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,2,0.01,5.05,self.iteration)
        # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,2,0.01,5.05,self.iteration)
        ...
        # self.prox_dgd_consensus_graph(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,2/self.m,0.01,5.05/self.m,self.iteration,graph,w_all)
        ...
        plt.ylabel("Average Distance from Node Average (dB)",fontsize=13)
        plt.grid(which = "major")
        # pdf.savefig()
        plt.legend(fontsize=13)
        plt.savefig('consensus graph.pdf')
        plt.show()
        # x  = range(len(w_star))

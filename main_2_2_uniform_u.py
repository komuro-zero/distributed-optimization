...
class distributed_updates(update_functions):
    ...
    def run(self):
        ...
        # self.centralized_gradient_descent(U_all,d_all,w,w_star,L2,0.0038,self.iteration)
        # error,wcmc = self.centralized_L1(U_all,d_all,w,w_star,L2,0.15,0.0038,self.iteration)
        
        
        error_centralized_mc,wcmc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.19,0.0038,0.3,self.iteration)
        error_centralized_dual_soft,wcmc = self.centralized_mc_twin(U_all,d_all,w,w_star,L2,0.19,0.0038,0.3,self.iteration,self.m)

        self.pg_extra_l1(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.19/self.m,0.082,0.09,self.iteration,graph,w_all)
        self.pg_extra_mc(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.19/self.m,0.082,0.3/self.m,self.iteration,graph,w_all)
        self.pg_extra_mc_soft(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.19/self.m,0.082,0.3/self.m,self.iteration,graph,w_all)


        # error,w_mc = self.centralized_mc(U_all,d_all,w,w_star,L2,0.19,0.0038,0.3,self.iteration)
        # error,wcmc = self.centralized_mc_twin_nonconvex(U_all,d_all,w,w_star,L2,0.19,0.0038,0.3,self.iteration,self.m)
        # error,wcmc = self.centralized_mc_twin(U_all,d_all,w,w_star,L2,0.17,0.0038,0.3,self.iteration,self.m)
        ...
        # self.distributed_proximal_gradient_algorithm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.15/self.m,0.035,0.00007/self.m,self.iteration,graph,w_all)
        # self.distributed_proximal_gradient_algorithm_firm(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.19/self.m,0.035,0.3/self.m,self.iteration,graph,w_all)
        # self.distributed_proximal_gradient_algorithm_approximate_MC(U_all,d_all,w_star,L2,self.N,self.m,self.r_i,0.19/self.m,0.035,0.3/self.m,self.iteration,graph,w_all)
        ...
        plt.axhline(y = error_centralized_mc[-1],linestyle="dashed",label = "Centralized MC penalty",color = "orange")
        plt.axhline(y = error_centralized_dual_soft[-1],linestyle = "dashed",label = "Centralized Approximate MC penalty",color = "green")
        pdf.savefig()
        plt.legend()
        plt.show()
        x  = range(len(w_star))
